"""
isbank_2026_scraper.py
==========================
İş Bank 2026 - tum 15 kolon.
...
Calistir: python isbank_2026_scraper.py
Cikti:    isbank_2026.xlsx  +  isbank_2026.csv
...
# ═══════════════════════════════════════════════════════════════
# BANKA DEGISTIRMEk ICIN SADECE BU BOLUMU DUZENLE
# ═══════════════════════════════════════════════════════════════
#
# Ornek bankalar:
#   Kuveyt Türk    → slug="kuveyt-turk"
#   Kuveyt Turk  → slug="kuveyt-turk"
#   Garanti      → slug="garanti-bankasi"
#   Ziraat       → slug="ziraat-bankasi"
#   Is Bankasi   → slug="turkiye-is-bankasi"
#   Yapi Kredi   → slug="yapi-kredi"
#   Akbank       → slug="akbank"
#   Halkbank     → slug="halkbank"
#   Denizbank    → slug="denizbank"
#
# Slug bulmak icin: sikayetvar.com'da bankayı arayip URL'e bak
# Ornek: sikayetvar.com/kuveyt-turk → slug = "kuveyt-turk"
#
OPERATOR   = "İş Bank"          # ← Banka adi (Excel'de gorunur)
SLUG       = "is-bankasi"          # ← URL slug (sikayetvar.com/SLUG)
...
OUTPUT_CSV   = f"{SLUG}_{DATE_START.year}.csv"
# ═══════════════════════════════════════════════════════════════

# Kapsam stratejisi:
#
#   resolved=true   → Cozulmus sikayetler   (tum yil)
#   resolved=false  → Cozulmemis sikayetler (tum yil)
#
# Bu iki endpoint birlikte tum sikayetleri kapsar — hic biri atlanmaz.
# Her biri kendi 1-200 sayfa havuzuna sahip.
# seen_ids seti ile duplicate'ler otomatik temizlenir.
#
# Ek endpointler: resolved=true/false 200 sayfa sinirini asarsa
# (yani 4800'den fazla cozulmus veya cozulmemis sikayet varsa)
# asagidaki yedek endpointler devreye girer.
ENDPOINTS = [
    ("resolved_true",  "resolved=true"),
    ("resolved_false", "resolved=false"),
    # Yedek: resolved ile yakalanamamis sikayetler icin
    ("date_asc",       "sortField=date&sortOrder=asc"),
    ("date_desc",      "sortField=date&sortOrder=desc"),
]
            ...
            for c in cards:
                ...
                if in_window(d):
                    if c.id not in seen_ids:
                        seen_ids.add(c.id)
                        # Endpoint'ten is_resolved bilgisini doldur
                        if ep_name == "resolved_true" and not c.is_resolved:
                            c.is_resolved = "Çözüldü"
                        elif ep_name == "resolved_false" and not c.is_resolved:
                            c.is_resolved = "Çözülmedi"
                        pending.append(c)
                        n_in += 1
                        ep_total += 1
                    else:
...
if __name__ == "__main__":
    log.info("=" * 60)
    log.info("İş Bank 2026 Scraper — 15 Kolon | 5 Endpoint")
